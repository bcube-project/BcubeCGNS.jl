function get_nodes_from_predicate(root, condition; depth = typemax(Int), kwargs...)
    result = CGNSNode[]
    _get_nodes_from_predicate!(result, root, condition, depth)
    return result
end

function _get_nodes_from_predicate!(result, root, condition, depth)
    condition(root) && push!(result, root)
    if depth > 0
        foreach(get_children(root)) do child
            _get_nodes_from_predicate!(result, child, condition, depth - 1)
        end
    end
end
